...
export default function HomePage({ setUrl, setHeaders }) {
  ...
  // Keep a ref so the useEffect can read the latest customHeaders without
  // needing it as a dependency (avoids re-running on every header change)
  ...
  async function validateAndSetUrl(url, headers = customHeaders) {
    toast.loading(`Validating...`, { duration: 800 });
    let data = await parseHls({ hlsUrl: url, headers });
    if (!data) {
  ...
  // Auto-trigger download when ?url= is present in the page URL
  useEffect(() => {
    const params = new URLSearchParams(window.location.search);
    try {
      if (params.has("url")) {
        // Fix 1: use url.href (string) not the URL object itself
        const url = new URL(params.get("url"));
        const href = url.href;

        // Fix 2: set text as string so the input displays correctly
        setText(href);

        // Fix 3: pass headers directly instead of relying on state
        // which may not be initialised yet at this point
        validateAndSetUrl(href, customHeadersRef.current);
      }
    } catch (error) {
      // Invalid URL in query string — just ignore it
    }
  }, []);
